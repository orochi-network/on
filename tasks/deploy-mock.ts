...
task("deploy-mock", "Deploy mock contracts").setAction(
  async (_args, hre) => {
    ...
    const MockVestingMain = await hre.ethers.getContractFactory("MockVestingMain");
    ...
    const onAirdrop = await ONAirdrop.connect(deployer).deploy(mockVestingMain);
    ...
    await (await mockVestingMain.transfer(onAirdrop, parseEther("20000000"))).wait(CONFIRMATION);

    await (await onAirdrop.addRecipient([beneficiary], [parseEther("1000")])).wait(CONFIRMATION);
