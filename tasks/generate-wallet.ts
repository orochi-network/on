...
task('create:wallet', 'Create new wallet').setAction(async (_taskArgs: any) => {
  ...
  console.log(
    'Generate new wallet with address:',
    wallet.address,
    'path:',
    wallet.path
  );
  const aes = await EncryptionKey.getInstance();
  const encryptedBase64 = aes
    .encrypt(Buffer.from(phrase, 'utf-8'))
    .toString('base64');
  let content = '';
  if (existsSync('.env')) {
  ...
  const decryptedPassPhrase = aes
    .decrypt(Buffer.from(encryptedBase64, 'base64'))
    .toString('utf-8');
